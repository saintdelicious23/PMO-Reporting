...
import { downloadPortfolioPdf, type PortfolioPdfKind } from "./pdf-report.ts";
...
function shell() {
  ...
  app.innerHTML = `
    ...
    <dialog id="report-dialog" class="modal report-modal"><div class="modal-head"><div><p class="eyebrow">Izvoz podataka</p><h2>Izaberi vrstu pregleda</h2></div><button type="button" class="close-button" data-close-dialog="report-dialog" aria-label="Zatvori">×</button></div><p class="report-dialog-intro">PDF više ne kopira ekransku tabelu. Svaki format je projektovan za svoju namenu.</p><div class="report-choices"><button type="button" class="report-choice" data-generate-report="executive"><span class="report-choice-kicker">BRZ PREGLED</span><strong>Direktorski pregled</strong><p>„Sveden“ pregled dopunjen komentarom poslednjeg preseka i jasnim signalima za reakciju.</p><small>Čitljiva tabela · A4 položeno</small></button><button type="button" class="report-choice" data-generate-report="detail"><span class="report-choice-kicker">TEMELJNO ČITANJE</span><strong>Detaljni pregled projekata</strong><p>Sva raspoloživa polja složena u jasne sekcije, bez izbacivanja informacija.</p><small>Projektni dosije · A4 uspravno</small></button></div><div class="modal-actions"><button type="button" class="button ghost" data-close-dialog="report-dialog">Otkaži</button></div></dialog>
...
function statusForm(p:ProjectDetail) { return `<form id="status-form"><section class="editor-section"><div class="section-title"><div><p class="eyebrow">Novi presek</p><h3>Upravljački status</h3></div><span class="updated-label">${statusUpdateLabel(p.lastStatusAt)}</span></div><div class="form-grid">
  ...
  <label class="check-field span-3 rose"><input type="checkbox" name="managementAttention" ${p.managementAttention?"checked":""}/><span><b>Reakcija menadžmenta</b><small>Naglasi projekat u upravljačkom pregledu</small></span></label><label class="field span-3"><span>Statusni presek - komentar</span><textarea name="summary" placeholder="Šta je urađeno, šta nije i šta sledi"></textarea></label>
...
function bindEvents() {
  ...
  document.querySelector("#print-report")!.addEventListener("click",()=>document.querySelector<HTMLDialogElement>("#report-dialog")!.showModal());
  document.querySelectorAll<HTMLElement>("[data-generate-report]").forEach(button=>button.addEventListener("click",()=>printReport(button.dataset.generateReport as PortfolioPdfKind)));
...
async function printReport(kind:PortfolioPdfKind){const trigger=document.querySelector<HTMLButtonElement>(`[data-generate-report="${kind}"]`)!;trigger.disabled=true;const title=trigger.querySelector("strong")!,original=title.textContent;title.textContent="Generišem PDF…";try{const visible=filteredProjects();const reportProjects=kind==="executive"?visible.filter(project=>!["completed","cancelled"].includes(project.lifecycleStatus)):portfolioSettings.pdfIncludeInactive?visible:visible.filter(project=>!["completed","cancelled"].includes(project.lifecycleStatus));const reportGroup=portfolioSettings.pdfGroup==="current"?groupMode:portfolioSettings.pdfGroup as GroupMode;const excludesInactive=kind==="executive"||!portfolioSettings.pdfIncludeInactive;const summary=`${excludesInactive?`${filterSummary()} · Samo aktivni projekti`:filterSummary()}${showDemo?"":" · Probni podaci isključeni"}`;const histories=kind==="detail"?Object.fromEntries(await Promise.all(reportProjects.map(async project=>[project.id,(await loadStatusHistory(project.id)).slice(0,4)] as const))):undefined;await downloadPortfolioPdf(reportProjects,kind,reportGroup,summary,portfolioSettings,histories);document.querySelector<HTMLDialogElement>("#report-dialog")!.close();toast(kind==="executive"?"Direktorski pregled je preuzet":"Detaljni pregled je preuzet");}catch(error){toast(error instanceof Error?error.message:"PDF nije generisan","error");}finally{trigger.disabled=false;title.textContent=original;}}
